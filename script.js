  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(PASSWORD_STORE_NAME)) {
        db.createObjectStore(PASSWORD_STORE_NAME, { keyPath: 'id', autoIncrement: true });
      }
    };
...
async function getMasterKeyFromDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    getRequest.onsuccess = () => { resolve(getRequest.result?.rawKey || null); };
...
function generateSecurePassword(options) {
  ...
  if (!charPool) { throw new Error('Не выбрано ни одного набора символов для пароля!'); }
